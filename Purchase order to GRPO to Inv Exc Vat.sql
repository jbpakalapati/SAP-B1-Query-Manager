...
AS "PO STATUS",
(PO."DocTotal" - PO."VatSum") AS "Total Before Tax", 
PO."VatSum", 
PO."DocTotal" AS "PO TOTAL",
 PO."Comments" AS "PO Remarks",
PO."U_SalesQuoteRef" as "Sales Quote Reference",
 PO."U_Division" as "Division", (SELECT div."OcrName" FROM OOCR div WHERE div."OcrCode" =PO."U_Division") as "Division Name",
...
(SELECT prj."PrjName" FROM OPRJ prj WHERE prj."PrjCode" = PO."U_Contract") as "Contract Name",
...
(SELECT divv."PrcName" FROM OPRC divv WHERE divv."PrcCode" =(SELECT  STRING_AGG("Project",',') FROM (select DISTINCT "Project" from POR1 K WHERE K."DocEntry"= PO."DocEntry" AND "Project"<>''  GROUP BY "Project" ))) as "Project Name",
...
AS "GRN STATUS",
(G."DocTotal" - G."VatSum") AS "Total Before Tax", 
G."VatSum", 
...
---IV."DocStatus" 
AS "INV STATUS",
(IV."DocTotal" - IV."VatSum") AS "Total Before Tax", 
IV."VatSum", 
IV."DocTotal" AS "INV TOTAL",
IV."Comments" AS "INV Remarks"

 from
OPOR PO left JOIN POR1 P1 ON PO."DocEntry"=P1."DocEntry"
...
WHERE PO."DocDate" between '[%1]' and '[%2]' 
order by  PO."DocEntry"
----,G."DocEntry",IV."DocEntry";